...
from utils.elastic_weight_consolidation import ElasticWeightConsolidation
...
parser.add_argument('--old_subject', type=str, default=[f'S12', 'S16', 'S29', 'S39', 'S40', 'S13', 'S2', 'S28'])
parser.add_argument('--new_subject', type=str, default=[f'S30', 'S36', 'S3'])
# [f"S{i+1}" for i in range(40)]
parser.add_argument('--source_name', type=str, default="E1-E2EC-E3EC-E4EC")
parser.add_argument('--target_name', type=str, default="E5EC")
...
source_subject = args.old_subject
...
if __name__ == '__main__':
    ...
    train_dataset_list = []
    for i, each_subject in enumerate(source_subject):
        ...
        train_dataset = ninapro.NinaPro(emgtrain_dir, glovetrain_dir, window_size=200, subframe=args.subframe,
                                        normalization=args.normalization, mu=args.miu, dummy_label=old_cls_num-len(source_subject)+i,
                                        class_num=old_cls_num)
    ...
    model = model.cuda()
    ewc_trainer = ElasticWeightConsolidation(model=model, lr=args.lr, weight=1e7, freeze=False)
    ewc_trainer.register_ewc_params(train_dataset_list, batch_size=4, num_batches=200)
    ...
    scheduler = MultiStepLR(ewc_trainer.optimizer, milestones=args.milestones, gamma=0.5)  # learning rates
    # ewc_trainer = ElasticWeightConsolidation(model=model, crit=criterion, lr=args.lr, weight=1e4, freeze=True)
    ...
    for epoch in range(initial_epoch, args.epoch):
        ...
        for n_count, batch_tr in enumerate(TrainLoader):
            ...
            y_pred, y_c_shuffle, x_hat_r, x_hat_c = model(batch_emg)
            # y_pred, y_c, y_s, _, z_s, z_c = model(batch_emg)

            # loss = 0.01 * reg_loss(y_s, y_s_label) + 0.99 * reg_loss(y_s_remember, y_s_old)
            ewc_trainer.forward_backward_update(batch_emg,batch_glove,y_s_label)

            loss = ewc_trainer.loss
        ...
        with torch.no_grad():
            ...
            for _, batch_eval in enumerate(DLoader_eval):
                ...
                glove_pred = ewc_trainer.model.inference(batch_eval[0].squeeze(3).permute(0, 2, 1).cuda(), )
        ...
        if NRMSE < best_epoch['NRMSE']:
            ...
            torch.save(ewc_trainer.model, os.path.join(save_dir, 'model_best.pth'))
        ...
        torch.save(ewc_trainer.model, os.path.join(save_dir, 'model_latest.pth'))
