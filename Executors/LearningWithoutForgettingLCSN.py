...
parser.add_argument('--new_subject', type=str, default=[f'S21', 'S1', 'S7', 'S22', 'S3', 'S28', 'S5', 'S23', 'S31'])
# [f"S{i+1}" for i in range(40)]
parser.add_argument('--source_name', type=str, default="A1-A2LF-A3LF")
parser.add_argument('--target_name', type=str, default="A4LF")
...
if __name__ == '__main__':
    ...
    for i, each_subject in enumerate(subject):
        ...
        train_dataset = ninapro.NinaPro(emgtrain_dir, glovetrain_dir, window_size=200, subframe=args.subframe,
                                        ...
                                        class_num=new_cls_num + old_cls_num)
    ...
    target_model.gen_encoder.extend_capsules(new_cls_num)
    target_model.shared_cls_head.extend_cls(new_cls_num)
    ...
    for epoch in range(initial_epoch, args.epoch):
        ...
        for n_count, batch_tr in enumerate(TrainLoader):
            ...
            y_source, y_c_source, x_hat_r_s, x_hat_c_s = source_model(batch_emg)
            ...
            loss = reg_loss(y_pred, batch_glove) + reg_loss(x_hat_r, batch_emg) + reg_loss(x_hat_c, batch_emg) + 1e4 * (
                cls_loss(y_c_shuffle, torch.max(y_s_label, dim=1)[1]))

            kd_loss = reg_loss(y_pred, y_source) + reg_loss(x_hat_r, x_hat_r_s) + reg_loss(x_hat_c, x_hat_c_s) + 1e4 * (
                cls_old_loss(y_c_shuffle[:, :old_cls_num], y_c_source, 2))
