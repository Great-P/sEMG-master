...
parser.add_argument('--source_name', type=str, default="E1-E2FT-E3FT-E4FT")
parser.add_argument('--target_name', type=str, default="E5FT")
...
if __name__ == '__main__':
    ...
    if new_cls_num>old_cls_num:
        model.gen_encoder.extend_capsules(new_cls_num-old_cls_num)
        model.shared_cls_head.extend_cls(new_cls_num-old_cls_num)
    ...
    for epoch in range(initial_epoch, args.epoch):
        ...
        for n_count, batch_tr in enumerate(TrainLoader):
            ...
            loss = reg_loss(y_pred, batch_glove) + reg_loss(x_hat_r, batch_emg) + reg_loss(x_hat_c,
                                                                                            batch_emg) + 1e4 * (
