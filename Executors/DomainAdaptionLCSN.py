...
import shutil
from utils.methods import str2bool
...
from utils.GenMiningNet.GenMiningNet import GeneralityMiningNetwork, SharedClassificationHead
from utils.GenMiningNet.LifelongCrossSubjectNetwork import LifelongCrossSubjectNetwork
...
parser.add_argument('--source_name', type=str, default="E1-E2DA-E3DA-E4DA")
parser.add_argument('--target_name', type=str, default="E5DA")
...
if __name__ == '__main__':
    ...
    source_model = LifelongCrossSubjectNetwork(vocab_size=args.subframe, hidden=args.hidden, class_num=len(subject),
                                               joint_channels=10, emg_channels=12)
    target_model = LifelongCrossSubjectNetwork(vocab_size=args.subframe, hidden=args.hidden, class_num=len(subject),
                                               joint_channels=10, emg_channels=12)
    # model.check()
    ...
    cur_model_name = target_model.name
    if args.smooth:
    ...
    source_model = torch.load(source_dir)
    target_model = torch.load(source_dir)
    old_cls_num = target_model.shared_cls_head.classnum
    ...
    for i, each_subject in enumerate(subject):
        ...
        train_dataset = ninapro.NinaPro(emgtrain_dir, glovetrain_dir, window_size=200, subframe=args.subframe,
                                        ...
                                        class_num=new_cls_num if new_cls_num > old_cls_num else old_cls_num)
    ...
    if new_cls_num > old_cls_num:
        target_model.gen_encoder.extend_capsules(new_cls_num - old_cls_num)
        target_model.shared_cls_head.extend_cls(new_cls_num - old_cls_num)
    ...
    record_writer.add_text("Model Scale", summary(target_model, (12, args.subframe, 1), args.batch_size))
    # add log
    ...
    for epoch in range(initial_epoch, args.epoch):
        ...
        for n_count, batch_tr in enumerate(TrainLoader):
            ...
            y_pred, y_c_shuffle, x_hat_r, x_hat_c = target_model(batch_emg)
            # y_pred, y_c, y_s, _, z_s, z_c = model(batch_emg)
            source_z_r = source_model.gen_encoder.feature_extractor(batch_emg)
            source_z_c = source_model.chara_encoder(batch_emg)

            target_z_r = target_model.gen_encoder.feature_extractor(batch_emg)
            target_z_c = target_model.chara_encoder(batch_emg)

            # loss = 0.01 * reg_loss(y_s, y_s_label) + 0.99 * reg_loss(y_s_remember, y_s_old)
            loss = reg_loss(y_pred, batch_glove) + reg_loss(x_hat_r, batch_emg) + reg_loss(x_hat_c, batch_emg) + 1e4 * (
                cls_loss(y_c_shuffle, torch.max(y_s_label, dim=1)[1])) + kl_loss(source_z_r, target_z_r) + kl_loss(
                source_z_c, target_z_c)
        ...
        with torch.no_grad():
            ...
            for _, batch_eval in enumerate(DLoader_eval):
                ...
                glove_pred = target_model.inference(batch_eval[0].squeeze(3).permute(0, 2, 1).cuda(), )
        ...
        if NRMSE < best_epoch['NRMSE']:
            ...
            torch.save(target_model, os.path.join(save_dir, 'model_best.pth'))
        ...
        torch.save(target_model, os.path.join(save_dir, 'model_latest.pth'))
